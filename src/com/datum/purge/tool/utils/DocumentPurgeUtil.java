...
public class DocumentPurgeUtil {
	...
	private Domain fetchFileNetConnection() throws Exception {
		...
		try {
			...
			subject = UserContext.createSubject(connection, DocumentPurgeConfigLoader.ce_UserID,
					DocumentPurgeConfigLoader.ce_Password, null);
			...
			DocumentPurgeLogger.writeLog(className, methodName, DocumentPurgeLogger.DEBUG,
					"FileNet Connection has been established.");
	...
	public ObjectStore fetchObjectStore() throws Exception {
		...
		return Factory.ObjectStore.fetchInstance(fetchFileNetConnection(), DocumentPurgeConfigLoader.ce_ObjectStoreName,
				null);
	}
